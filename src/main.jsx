...
ReactDOM.createRoot(document.getElementById('root')).render(
  <React.StrictMode>
  {/* Popup saying to safari users hide the search bar */}
<div className="modal" id="safari-warning">
  <div className="modal-box bg-warning">
    <h3 className="font-bold text-lg">Problemas de compatibilidade.</h3>
<p className="py-4">Usuarios Safari, no nosso site existe um problema de responsividade com o seu navegador, para corrigir esse problema é necessario que a barra de ferramentas do Safari seja ocultada. Para isso basta apertar no botão <a className="font-bold">Aa</a> no canto superior esquerdo da tela e em seguida apertar <a className="font-bold">Ocultar barra de ferramenta</a>.</p>
    <div className="modal-action">
     <a href="#" className="btn bg-success font-white">Pronto!</a>
    </div>
  </div>
</div>
    ...
    <div name="comeco" id="comeco" className="flex center justify-center flex-col h-screen space-y-5 items-center">
      ...
      <Textbox classesName="absolute bottom-0" textSize="text-xs" content="Copyright © 2022 - mateusdccavalcanti@gmail.com"/>
    </div>
  {/* Twitter */}
    <div id="setw" name="setw" className="bg-twitter flex flex-col h-screen space-y-5 relative ">
      ...
      <a className="btn rounded-none btn-accent border-0 no-animation black-transparent-bg-30 w-auto h-1/6 font-transparent font-anton text-4xl mt-auto" href="#sein">Ir para Instagram</a>
    </div>
  {/* Instagram */}
    <div id="sein" name="sein" className="bg-instagram flex flex-col h-screen space-y-5 relative ">
      ...
      <a className="btn rounded-none btn-accent border-0 no-animation black-transparent-bg-30 w-auto h-1/6 font-transparent font-anton text-4xl absolute bottom-0 right-0 left-0" href="#setik">Ir para Tik Tok</a>
    </div>
  {/* TikTok */}
    <div id="setik" name="setik" className="bg-tiktok flex flex-col h-screen space-y-5 relative ">
      ...
      <a className="btn rounded-none btn-accent border-0 no-animation black-transparent-bg-30 w-auto h-1/6 font-transparent font-anton text-4xl absolute bottom-0 right-0 left-0" href="#comeco">Ir para começo</a>
    </div>
  {/* Twitter content */}
...
if (navigator.userAgent.indexOf('Safari') != -1 && navigator.userAgent.indexOf('Chrome') == -1) {
  setTimeout(function() {
    window.location.href = '#safari-warning';
  }, 1000);
}
